...
export function resolvePackageVersion(importMetaUrl) {
    const injected = typeof process !== 'undefined' && typeof process.env.SLASH_VERSION === 'string'
        ? process.env.SLASH_VERSION.trim()
        : '';
...
export function resolveGitSha(importMetaUrl) {
    const injected = typeof process !== 'undefined' && typeof process.env.SLASH_GIT_SHA === 'string'
        ? process.env.SLASH_GIT_SHA.trim()
        : '';
    if (injected.length > 0) {
