...
import { TWITTER_MEDIA_METADATA_URL, TWITTER_UPLOAD_URL } from './constants.js';
...
export function withMedia<TBase extends AbstractConstructor<TwitterClientBase>>(Base: TBase): Mixin<TBase, TwitterClientMediaMethods> {
    abstract class TwitterClientMedia extends Base {
        ...
        mediaCategoryForMime(mimeType: string): string | null {
        ...
        async uploadMedia(input: { data: Uint8Array; mimeType: string; alt?: string }): Promise<UploadMediaResult> {
            ...
            try {
                ...
                // biome-ignore lint/suspicious/noExplicitAny: processing_info shape
                const finalizeBody = (await finalizeResp.json()) as { processing_info?: any };
                ...
                if (info?.state && info.state !== 'succeeded') {
                    ...
                    while (attempts < 20) {
                        ...
                        // biome-ignore lint/suspicious/noExplicitAny: processing_info shape
                        const statusBody = (await statusResp.json()) as { processing_info?: any };
    ...
    return TwitterClientMedia as unknown as Mixin<TBase, TwitterClientMediaMethods>;
}
