...
export function withEngagement<TBase extends AbstractConstructor<TwitterClientBase>>(Base: TBase): Mixin<TBase, TwitterClientEngagementMethods> {
    abstract class TwitterClientEngagement extends Base {
        ...
        async performEngagementMutation(operationName: 'FavoriteTweet' | 'UnfavoriteTweet' | 'CreateRetweet' | 'DeleteRetweet' | 'CreateBookmark', tweetId: string): Promise<BookmarkMutationResult> {
            ...
            const buildBody = (): string => JSON.stringify({ variables, queryId });
            const buildHeaders = (): Record<string, string> => ({ ...this.getHeaders(), referer: `https://x.com/i/status/${tweetId}` });
            let body = buildBody();
    ...
    return TwitterClientEngagement as unknown as Mixin<TBase, TwitterClientEngagementMethods>;
}
