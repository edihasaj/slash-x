...
export function withUserLookup<TBase extends AbstractConstructor<TwitterClientBase>>(Base: TBase): Mixin<TBase, TwitterClientUserLookupMethods> {
    abstract class TwitterClientUserLookup extends Base {
        ...
        async getUserByScreenNameGraphQL(screenName: string): Promise<UserLookupResult> {
        ...
        async getUserIdByUsername(username: string): Promise<UserLookupResult> {
            ...
            for (const url of urls) {
                try {
                    ...
                    const data = (await response.json()) as { id_str?: string; id?: string | number; screen_name?: string; name?: string };
        ...
        async getAboutAccountQueryIds(): Promise<string[]> {
    ...
    return TwitterClientUserLookup as unknown as Mixin<TBase, TwitterClientUserLookupMethods>;
}
