import type { GraphqlTweetResult, TweetData, TweetMedia, TwitterUser } from './types.js';
...
export function extractArticleText(result: GraphqlTweetResult | undefined): string | undefined {
    ...
    if (process.env.SLASH_DEBUG_ARTICLE === '1') {
        console.error('[slash][debug][article] payload:', JSON.stringify({
    ...
    const contentState = article.article_results?.result?.content_state as ContentState | undefined;
...
export function extractArticleMetadata(result: GraphqlTweetResult | undefined): { title: string; previewText?: string } | undefined {
    const article = result?.article;
    if (!article) {
        return undefined;
    }
...
export function mapTweetResult(result: GraphqlTweetResult | undefined, quoteDepthOrOptions: number | MapTweetResultOptions): TweetData | undefined {
    const options: MapTweetResultOptions = typeof quoteDepthOrOptions === 'number' ? { quoteDepth: quoteDepthOrOptions } : quoteDepthOrOptions;
...
export function findTweetInInstructions(instructions: Array<{
    entries?: Array<{
        content?: {
            itemContent?: {
                tweet_results?: {
...
export function collectTweetResultsFromEntry(entry: {
    ...
    const pushResult = (result: GraphqlTweetResult | undefined): void => {
        if (result?.rest_id) {
            results.push(result);
        }
    };
...
// biome-ignore lint/suspicious/noExplicitAny: deeply nested instruction type
export function parseTweetsFromInstructions(instructions: any[] | undefined, quoteDepthOrOptions: number | ParseTweetsOptions): TweetData[] {
    const options: ParseTweetsOptions = typeof quoteDepthOrOptions === 'number' ? { quoteDepth: quoteDepthOrOptions } : quoteDepthOrOptions;
    const { quoteDepth, includeRaw = false } = options;
...
export function extractCursorFromInstructions(instructions: Array<{
    entries?: Array<{
        content?: unknown;
    }>;
}> | undefined, cursorType = 'Bottom'): string | undefined {
    for (const instruction of instructions ?? []) {
        for (const entry of instruction.entries ?? []) {
            // biome-ignore lint/suspicious/noExplicitAny: cursor entries have varied shapes
...
// biome-ignore lint/suspicious/noExplicitAny: user instruction shapes vary
export function parseUsersFromInstructions(instructions: any[] | undefined): TwitterUser[] {
    if (!instructions) {
        return [];
    }
