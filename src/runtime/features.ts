...
import defaultOverrides from '../data/features.json' with { type: 'json' };
...
function mergeOverrides(base: NormalizedFeatureOverrides, next: NormalizedFeatureOverrides): NormalizedFeatureOverrides {
...
function resolveFeaturesCachePath(): string {
    const override = process.env.SLASH_FEATURES_CACHE ?? process.env.SLASH_FEATURES_PATH;
...
function readOverridesFromEnv(): NormalizedFeatureOverrides | null {
    const raw = process.env.SLASH_FEATURES_JSON;
