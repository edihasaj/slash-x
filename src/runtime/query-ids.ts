...
function resolveDefaultCachePath(): string {
    const override = process.env.SLASH_QUERY_IDS_CACHE;
    if (override && override.trim().length > 0) {
        return path.resolve(override.trim());
    }
    return path.join(homedir(), '.config', 'slash-x', DEFAULT_CACHE_FILENAME);
}
function parseSnapshot(raw: unknown): RuntimeQueryIdSnapshot | null {
    ...
    const discovery = record.discovery && typeof record.discovery === 'object' ? (record.discovery as Record<string, unknown>) : null;
    if (!fetchedAt || !ttlMs || !ids || !discovery) {
        return null;
    }
    const pages = Array.isArray(discovery.pages) ? (discovery.pages as unknown[]) : null;
    const bundles = Array.isArray(discovery.bundles) ? (discovery.bundles as unknown[]) : null;
    if (!pages || !bundles) {
        return null;
    }
...
function extractOperations(bundleContents: string, bundleLabel: string, targets: Set<string>, discovered: Map<string, { queryId: string; bundle: string }>): void {
...
async function fetchAndExtract(fetchImpl: typeof fetch, bundleUrls: string[], targets: Set<string>): Promise<Map<string, { queryId: string; bundle: string }>> {
...
export function createRuntimeQueryIdStore(options: RuntimeQueryIdsOptions = {}): RuntimeQueryIdStore {
    ...
    const refresh = async (operationNames: string[], opts: { force?: boolean } = {}): Promise<RuntimeQueryIdSnapshotInfo | null> => {
        if (refreshInFlight) {
...
export const runtimeQueryIds: RuntimeQueryIdStore = createRuntimeQueryIdStore();
