...
import { hyperlink, labelPrefix, type OutputConfig, resolveOutputConfigFromArgv, resolveOutputConfigFromCommander, statusPrefix, } from '../lib/output.js';
...
function readConfigFile(path: string, warn: (message: string) => void): BirdConfig {
    if (!existsSync(path)) {
        return {};
    }
    try {
...
function loadConfig(warn: (message: string) => void): BirdConfig {
    const globalPath = join(homedir(), '.config', 'slash-x', 'config.json5');
    const localPath = join(process.cwd(), '.slashrc.json5');
    return {
...
export function createCliContext(normalizedArgs: string[], env: NodeJS.ProcessEnv = process.env): CliContext {
    ...
    function resolveTimeoutFromOptions(options: { timeout?: string | number }): number | undefined {
        return resolveTimeoutMs(options.timeout, config.timeoutMs, env.SLASH_TIMEOUT_MS);
    }
    function resolveCookieTimeoutFromOptions(options: { cookieTimeout?: string | number }): number | undefined {
        return resolveTimeoutMs(options.cookieTimeout, config.cookieTimeoutMs, env.SLASH_COOKIE_TIMEOUT_MS);
    }
    function resolveQuoteDepthFromOptions(options: { quoteDepth?: string | number }): number | undefined {
        return resolveQuoteDepth(options.quoteDepth, config.quoteDepth, env.SLASH_QUOTE_DEPTH);
    }
    ...
    function printTweets(tweets: TweetData[], opts: {
        json?: boolean;
        emptyMessage?: string;
        showSeparator?: boolean;
    } = {}): void {
        ...
        const mediaLabel = (type: string): string => {
            if (useEmoji) {
    ...
    function printTweetsResult(result: { tweets?: TweetData[]; nextCursor?: string }, opts: {
        json: boolean;
        usePagination: boolean;
        emptyMessage: string;
    }): void {
