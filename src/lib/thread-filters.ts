...
const sortByCreatedAt = (tweets: TweetData[]): TweetData[] => tweets.slice().sort((a, b) => {
...
export function filterFullChain(tweets: TweetData[], bookmarkedTweet: TweetData, options: {
    ...
    const addDescendants = (startIds: string[]): void => {
...
export function addThreadMetadata(tweet: TweetData, allConversationTweets: TweetData[]): TweetWithMeta {
    ...
    let threadPosition: TweetWithMeta['threadPosition'];
