...
export function registerUserTweetsCommand(program: Command, ctx: CliContext): void {
    const formatExample = (cmd: string, desc: string): string => `  ${ctx.colors.command(cmd)}\n    ${ctx.colors.muted(desc)}`;
    program
        .command('user-tweets')
        .description("Get tweets from a user's profile timeline")
        .argument('<handle>', 'Username to fetch tweets from (e.g., @hasajedi or hasajedi)')
        ...
        formatExample('slash user-tweets @hasajedi', 'Get recent tweets from a user'),
        formatExample('slash user-tweets hasajedi -n 10', 'Get 10 tweets (@ is optional)'),
        formatExample('slash user-tweets @hasajedi -n 50', 'Fetch 50 tweets (paged)'),
        formatExample('slash user-tweets @hasajedi --max-pages 3 -n 200', 'Safety cap (max 3 pages)'),
