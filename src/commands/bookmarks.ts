...
import type { TweetData } from '../twitter/types.js';
export function registerBookmarksCommand(program: Command, ctx: CliContext): void {
    program
        ...
        for (let index = 0; index < bookmarks.length; index += 1) {
            ...
            if (shouldFetchThread) {
                if (!expandRootOnly || isRoot || includeMeta) {
                    if (index > 0) {
                        await new Promise<void>((resolve) => setTimeout(resolve, delayBetweenExpansionsMs));
                    }
        ...
        let finalResults: any[] = expandedResults;
        if (includeMeta) {
