...
import type { AboutAccountProfile, FollowingResult, TwitterUser } from '../twitter/types.js';
...
async function resolveUserIdOrExit(client: TwitterClient, requestedUserId: string | undefined, ctx: CliContext): Promise<string> {
    if (requestedUserId) {
...
interface UserListSpec {
    name: string;
    description: string;
    fetch: (client: TwitterClient, userId: string, count: number, cursor?: string) => Promise<FollowingResult>;
}
// biome-ignore lint/suspicious/noExplicitAny: cmd opts shape
async function runUserListCommand(program: Command, ctx: CliContext, spec: UserListSpec, cmdOpts: any): Promise<void> {
...
export function registerUserCommands(program: Command, ctx: CliContext): void {
    const registerUserListCommand = (spec: UserListSpec): void => {
        program
            .command(spec.name)
            ...
            // biome-ignore lint/suspicious/noExplicitAny: cmd opts shape
            .action(async (cmdOpts: any) => runUserListCommand(program, ctx, spec, cmdOpts));
    };
    ...
    program
        ...
        .option('--json', 'Output as JSON')
        // biome-ignore lint/suspicious/noExplicitAny: cmd opts shape
        .action(async (username: string, cmdOpts: any) => {
        const opts = program.opts();
        const timeoutMs = ctx.resolveTimeoutFromOptions(opts);
