...
import { type FeatureOverrides, getFeatureOverridesSnapshot, refreshFeatureOverridesCache, } from '../runtime/features.js';
...
export function registerQueryIdsCommand(program: Command, ctx: CliContext): void {
    program
        ...
        if (!info) {
            ...
            console.log(`${ctx.p('info')}Run: slash query-ids --fresh`);
